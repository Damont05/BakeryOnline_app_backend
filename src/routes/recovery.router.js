...
const transport=nodemailer.createTransport(
      {
          ...
          port: 587, 
          auth: {
              user: "luisdanielmonterofalcon@gmail.com",
      ...
      let token=jwt.sign({...user}, "CoderCoder123", {expiresIn:"1h"})
          ...
          const enviar = () => {
              return transport.sendMail({
                  from: "Daniel Montero luisdanielmonterofalcon@gmail.com",
                  to: req.body.email,
  ...
  router.post("/recupero03",async(req,res)=>{
      ...
      try {
          ...
          let usuario=await usersModel.findOne({email:datosToken.email}).lean()
          if(!usuario){
              res.setHeader('Content-Type','application/json');
          ...
          await usersModel.updateOne({email:datosToken.email}, usuarioActualizado)
